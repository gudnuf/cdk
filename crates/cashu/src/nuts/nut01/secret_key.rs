...
impl SecretKey {
    ...
    /// Generate random secret key for WASM
    #[cfg(target_arch = "wasm32")]
    pub fn generate() -> Self {
        use getrandom::getrandom;
        let mut bytes = [0u8; 32];
        // Use getrandom which works reliably in WASM environments
        getrandom(&mut bytes).expect("Failed to generate random bytes for WASM");
        Self::from_slice(&bytes).expect("Generated bytes should be valid secret key")
    }
